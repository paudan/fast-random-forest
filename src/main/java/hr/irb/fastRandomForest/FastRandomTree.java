...
/**
 * Based on the "weka.classifiers.trees.RandomTree" class, revision 1.19, by
 * Eibe Frank and Richard Kirkby, with major modifications made to improve the
 * speed of classifier training.
 *
...
class FastRandomTree
    ...
    /**
     * Since 0.99: holds references to temporary arrays re-used by all nodes in
     * the tree, used while calculating the "props" for various attributes in
    ...
    /**
     * Since 0.99: holds references to temporary arrays re-used by all nodes in
     * the tree, used while calculating the "dists" for various attributes in
     * distributionSequentialAtt(). This is meant to avoid frequent
     * creating/destroying of these arrays.
     */
     ...
     * <p>
     *
     * The "data" field of the FastRandomTree should contain a reference to a
     * DataCache prior to calling this function, and that DataCache should have
     * the "reusableRandomGenerator" field initialized. The FastRfBagging class
     * normally takes care of this before invoking this function.
     */
    public void run() {
    ...
    public double[] distributionForInstance(Instance instance) throws Exception {
        ...
        if (m_Attribute > -1) {  // ============================ node is not a leaf
            ...
            } else if (m_MotherForest.m_Info.attribute(m_Attribute).isNominal()) 
     ...
     * <p>
     *
     * Main use of this is to compute out-of-bag error (also when finding
     * feature importances).
     *
     * @param instance the instance to compute the distribution for
     * @return the computed class distribution
     ...
     * </ul>
     *
     * @param sortedIndices the indices of the instances of the whole bootstrap
     * replicate
     * @param startAt First index of the instance to consider in this split;
     * inclusive.
    ...
    protected void buildTree(int[][] sortedIndices, int startAt, int endAt,
            double[] classProbs, boolean debug, int[] attIndicesWindow, int depth) {
        m_Debug = debug;
        ...
        while ((windowSize > 0) && (k-- > 0 || !sensibleSplitFound)) {
            ...
            if (Double.isNaN(candidateSplit))
                // we did not improve over a previous attribute! "dist" is unchanged from before
        ...
        if (sensibleSplitFound) {
            ...
            for (int i = 0; i < dist.length; i++) {
                ...
                if (belowTheSplitStartsAt - startAt == 0)
                    ...
                    // by the number of instances (as we won't be able to normalize after the split)
     ...
     * derivative of the splitData function from
     * "weka.classifiers.trees.RandomTree", with the following changes:
     * <p>
     *
     ...
     * FastRandomTree checks whether an instance's split attribute value was
     * above splitpoint only once per instances, and stores result into the
     * DataCache's whatGoesWhere field, which is then read in splitting
     * subsetIndices.
     * <p>
     *
     ...
     * @param att the attribute index
     * @param splitPoint the splitpoint for numeric attributes
     * @param sortedIndices the sorted indices of the whole set - gets
     * overwritten!
     * @param startAt Inclusive, 0-based index. Does not touch anything before
     * this value.
     * @param endAt Inclusive, 0-based index. Does not touch anything after this
     * value.
     *
     ...
     * relevant for the (comparative) goodness of a split is entropy after
     * splitting
     ...
     * middle' of instance 0, which would result in empty nodes after the split;
     * this is now fixed
     * <li>instance 0 is now generally skipped when looking for split points, as
     * the split point 'before instance 0' is not sensible; in versions prior to
     * 0.96 this change introduced a bug where attributes with all missing
     * values had their dists computed wrongly, which might result in useless
     * (but harmless) branches being added to the tree
     * </ul>
     *
     * @param props gets filled with relative sizes of branches (total = 1),
     * indexed first per attribute
     * @param dists these are the contingency matrices, indexed first per
     * attribute
     ...
     * <li> does not create a new "dists" for each attribute it examines;
     * instead it replaces the existing "dists" (supplied as a parameter) but
     * only if the split is better than the previous best split
     ...
     * </ul>
     *
     * @param propsBestAtt gets filled with relative sizes of branches (total =
     * 1) for the best examined attribute so far; updated ONLY if current
     * attribute is better that the previous best
     * @param distsBestAtt these are the contingency matrices for the best
     * examined attribute so far; updated ONLY if current attribute is better
     * that the previous best
     * @param scoreBestAtt Checked against the score of the attToExamine to
     * determine if the propsBestAtt and distsBestAtt need to be updated.
     * @param attToExamine the attribute index (which one to examine, and change
     * the above matrices if the attribute is better than the previous one)
     * @param sortedIndices the sorted indices of the vals for the attToExamine.
     * @param startAt Index in sortedIndicesOfAtt; do not touch anything below
     * this index.
     * @param endAt Index in sortedIndicesOfAtt; do not touch anything after
     * this index.
     */
    ...
    /**
     * Normalizes branch sizes so they contain frequencies (stored in "props")
     * instead of counts (stored in "dist"). Overwrites the supplied "props"!
