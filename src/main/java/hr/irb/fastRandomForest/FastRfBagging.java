...
/**
 * Based on the "weka.classifiers.meta.Bagging" class, revision 1.39, by Kirkby,
 * Frank and Trigg, with modifications:
 * <ul>
 *
 * <li>Instead of Instances, produces DataCaches; consequently, FastRfBagging is
 * compatible only with FastRandomTree as base classifier</li>
 *
 ...
 *
 * <li>The classifiers are trained in separate "tasks" which are handled by an
 * ExecutorService (the FixedThreadPool) which runs the tasks in more threads in
 * parallel. If the number of threads is not specified, it will be set
 * automatically to the available number of cores.
 * </li>
 *
 * <li>Estimating the out-of-bag (OOB) error is also multithreaded, using the
 * VotesCollector class</li>
 *
 * <li>OOB estimation in Weka's Bagging is one tree - one vote. In FastRF 0.97
 * onwards, some trees will have a heavier weight in the overall vote depending
 * on the averaged weights of instances that ended in the specific leaf.</li>
 *
 * </ul>
 *
...
class FastRfBagging extends RandomizableIteratedSingleClassifierEnhancer
     ...
     * FastRandomTree).
     *
     * @param data The training set to be used for generating the bagged
     * classifier.
    ...
    public String toString() {
        ...
        if (m_CalcOutOfBag)
            text.append("Out of bag error: ").append(Utils.doubleToString(m_OutOfBagError, 4)).append("\n\n");
